...
export class StandardMultiSigCreatePage implements OnInit {
  ...
  private async tryToAutoSelectedSigningWallet() {
    ...
    if (masterWallet && masterWallet.length === 1) await this.useSigningWallet(masterWallet[0]);
  }
  ...
  public allInputsValid(): boolean {
    if (!this.wallet.name) return false; // Need a wallet name

    if (!this.signingWallet) return false; // Need to have picked a signing wallet - no watch mode for now

    let reqSigners = new BigNumber(this.requiredSigners);
    if (reqSigners.isNaN() || !reqSigners.isInteger() || reqSigners.lte(0)) return false; // Need at least one signer, and need to be a valid number

    if (reqSigners.gt(this.getUsableCosigners().length + 1))
      // +1 because the user himself counts as as usable cosigners too
      return false; // Can't have more signers required than total cosigners

    // No invalid xpub
    for (let cosigner of this.cosigners) {
      if (!this.cosignerKeyIsValidIsValid(cosigner)) return false;
    }
  ...
  private async useSigningWallet(signingMasterWallet: MasterWallet) {
    ...
    this.cosigners = this.cosigners.map(c => {
      if (c.toLowerCase() === this.signingWalletXPub.toLowerCase()) return '';
      else return c;
    });
  }
  ...
  private isSigningWalletKey(key: string): boolean {
    if (!this.signingWalletXPub) return false;
