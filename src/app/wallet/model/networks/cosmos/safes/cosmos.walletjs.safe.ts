...
export class CosmosWalletJSSafe extends Safe implements CosmosSafe {
  ...
  public async initialize(networkWallet: AnyNetworkWallet): Promise<void> {
    ...
    if (!this.cosmosAddress) {
      ...
      if (this.cosmosAddress) await networkWallet.saveContextInfo('cosmosAddress', this.cosmosAddress);
    }
  }
  ...
  private async initClient() {
    ...
    let rpcUrl = this.networkWallet.network.getRPCUrl();
    // TODO: Unfixed gasPrice.
    this.signingStargateClient = await SigningStargateClient.connectWithSigner(rpcUrl, this.wallet, {
  ...
  private async getPrivateKey(forceShowMasterPrompt = false) {
    ...
    if (!payPassword) return null; // Can't continue without the wallet password - cancel the initialization
  ...
  public async signTransaction(
    ...
    if (this.signingStargateClient) {
      let result = await this.signingStargateClient.sendTokens(
        this.cosmosAddress,
        rawTransaction.to,
        coins(rawTransaction.value, 'uatom'),
        'auto'
      );
