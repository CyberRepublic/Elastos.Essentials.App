...
> {
  ...
  public extendedTransactionInfoUpdated = new Subject<{
    txHash: string;
    extInfo: ExtendedTransactionInfo;
  }>();
  ...
  public async initialize(): Promise<void> {
    ...
    await this.populateWithExtendedInfo(
  ...
  private getDisplayBalanceInCurrency(currencySymbol: string): BigNumber {
    if (!this.initializationComplete) return new BigNumber(0);
  ...
  public async createNonStandardSubWallet(coin: Coin): Promise<void> {
    try {
      ...
      if (err.code !== 20001) {
        // SPVSDK - 20001: Unsupported subwallet in some test networks.
        throw err;
      }
    }
  }
  ...
  public async save() {
    ...
    await LocalStorage.instance.setExtendedNetworkWalletInfo(
      this.id,
      GlobalNetworksService.instance.activeNetworkTemplate.value,
      this.network.key,
      extendedInfo
