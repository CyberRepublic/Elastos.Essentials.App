...
export class ElastosTokenSubWalletProvider extends SubWalletTransactionProvider<PGPSubWallet, EthTransaction> {
  ...
  public async discoverTokens(): Promise<void> {
    ...
    try {
      let tokenList = await GlobalElastosAPIService.instance.getERC20TokenList(StandardCoinName.ETHECOPGP, address);
