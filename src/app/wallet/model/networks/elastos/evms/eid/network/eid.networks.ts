  ...
  public async newNetworkWallet(masterWallet: MasterWallet): Promise<AnyNetworkWallet> {
    switch (masterWallet.type) {
      case WalletType.STANDARD:
        const ElastosIdentityChainStandardNetworkWallet = (
          await import('../networkwallets/standard/identitychain.networkwallet')
        ).ElastosIdentityChainStandardNetworkWallet;
        return new ElastosIdentityChainStandardNetworkWallet(masterWallet as StandardMasterWallet, this);
      case WalletType.LEDGER:
        const ElastosIdentityChainLedgerNetworkWallet = (
          await import('../networkwallets/ledger/identitychain.networkwallet')
        ).ElastosIdentityChainLedgerNetworkWallet;
...
export class ElastosIdentityChainMainNetNetwork extends ElastosIdentityChainNetworkBase {
  constructor() {
    super(
      ...
      [
        {
          name: 'Elastos Identity Chain RPC',
          url: GlobalElastosAPIService.instance.getApiUrl(
            GlobalElastosAPIService.instance.getApiUrlTypeForRpc(StandardCoinName.ETHDID),
            MAINNET_TEMPLATE
          )
        }
      ]
  ...
  public getAPIUrlOfType(type: NetworkAPIURLType): string {
    if (type === NetworkAPIURLType.RPC)
      ...
      return GlobalElastosAPIService.instance.getApiUrl(
        GlobalElastosAPIService.instance.getApiUrlTypeForBlockExplorer(StandardCoinName.ETHDID),
        MAINNET_TEMPLATE
      );
    } else return null;
  }
...
export class ElastosIdentityChainTestNetNetwork extends ElastosIdentityChainNetworkBase {
  constructor() {
    super(
      ...
      [
        {
          name: 'Elastos Identity Chain Testnet RPC',
          url: GlobalElastosAPIService.instance.getApiUrl(
            GlobalElastosAPIService.instance.getApiUrlTypeForRpc(StandardCoinName.ETHDID),
            TESTNET_TEMPLATE
          )
