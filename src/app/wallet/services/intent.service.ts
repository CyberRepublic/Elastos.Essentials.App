...
export class IntentService {
  ...
  addIntentListener() {
    this.subscription = this.globalIntentService.intentListener.subscribe(
      (receivedIntent) => {
  ...
  async handleTransactionIntent(intent: EssentialsIntentPlugin.ReceivedIntent) {
    if (Util.isEmptyObject(intent.params)) {
      Logger.error(
        "wallet",
        "Invalid intent parameters received. No params.",
        intent.params
      );
      await this.globalIntentService.sendIntentResponse(
    ...
    switch (this.getShortAction(intent.action)) {
      ...
      case "esctransaction":
        ...
        this.coinTransferService.sendTransactionChainId = intent.params.chainid; // optional - ok to be undefined
  ...
  handleAccessIntent(intent: EssentialsIntentPlugin.ReceivedIntent) {
    ...
    this.walletAccessService.requestFields =
      intent.params.reqfields || intent.params;
  ...
  private handleVoteAgainstProposalIntent(
    intent: EssentialsIntentPlugin.ReceivedIntent
  ) {
   ...
   */
  private async handleCreateProposalDigestIntent(
    intent: EssentialsIntentPlugin.ReceivedIntent
  ) {
    ...
    if (intent && intent.params && intent.params.proposal) {
      ...
      let mainChainSubwallet = activeNetworkWallet.getSubWallet(
        StandardCoinName.ELA
      ) as unknown as MainChainSubWallet;
      if (mainChainSubwallet) {
        let digest = await mainChainSubwallet.proposalOwnerDigest(
          intent.params.proposal
        );
  ...
  private async handlePayIntent(
    ...
    if (this.walletNetworkService.activeNetwork.value.key !== networkName) {
      ...
      let networkHasBeenSwitched =
        await this.globalSwitchNetworkService.promptSwitchToNetwork(network);
    ...
    const intentSubWalletId = this.getSubWalletIdByCurrency(
      intent.params.currency || "ELA"
    );
