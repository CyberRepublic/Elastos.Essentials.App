            ...
            res => {
              if (res && res.data) {
                try {
                  ...
                  reject(
                    'Invalid JSON response returned by the JSON RPC for url: ' +
                      url +
                      ', with params: ' +
                      paramsAsString
                  );
  ...
  httpGet(url: string, limitatorName = 'default', timeout = 10000, customHeaders?: { [key: string]: string }): Promise<any> {
    ...
    let promiseResult = limitator.queue.add(() => {
      return new Promise((resolve, reject) => {
        ...
        this.http.sendRequest(url, <any>options).then(
          res => {
            if (res && res.data) resolve(res.data); // Unblock the calling method
            else {
              Logger.error('GlobalJsonRPCService', 'http get response with error:', res, ' url:', url);
              reject('http get response with error');
            }
  ...
  httpDelete(url: string, limitatorName = 'default'): Promise<any> {
    ...
    let promiseResult = limitator.queue.add(() => {
      return new Promise((resolve, reject) => {
        ...
        this.http.sendRequest(url, <any>options).then(
          res => {
            if (res && !res.error) resolve(res.data); // Unblock the calling method
            else {
