 ...
 * - By default, ionic storage, which is actually configured to used the cordova sqlite storage. Use this for data that require permanent persistance. But slow.
...
export class GlobalStorageService {
  ...
  private async deleteDIDSettingsIonic(did: string, networkTemplate: string): Promise<void> {
    ...
    Logger.log(
      ...
      'Deleted ' +
        deletedEntries +
        ' ionic settings entries for DID ' +
        did +
        ' for network template ' +
        networkTemplate
    );
  }
  ...
  private deleteDIDSettingsLocalStorage(did: string, networkTemplate: string) {
    ...
    Logger.log(
      ...
      'Deleted ' +
        deletedEntries +
        ' local storage settings entries for DID ' +
        did +
        ' for network template ' +
        networkTemplate
    );
  ...
  public async setSetting<T>(
    ...
    key: string,
    value: T,
    location: StorageLocation = 'ionic'
  ): Promise<void> {
    ...
    if (location === 'ionic')
      void this.storage.set(fullKey, JSON.stringify(value)).then(
        res => {},
        err => {}
      );
  ...
  public async getSetting<T>(
    ...
    try {
      ...
      if (res === undefined) {
        if (location === 'ionic') res = await this.storage.get(fullKey);
        else if (location === 'browserlocalstorage') {
  ...
  public deleteSetting(
    ...
    if (location === 'ionic') void this.storage.remove(fullKey);
    else if (location === 'browserlocalstorage') localStorage.removeItem(fullKey);
