...
export class Util {
  ...
  public static isBankCard(bankCard: string): boolean {
    var regex =
      /^(998801|998802|622525|622526|435744|435745|483536|528020|526855|622156|622155|356869|531659|622157|627066|627067|627068|627069|622588)\d{10}$/;
  ...
  public static clone(Obj) {
    ...
    for (let i in Obj) newObj[i] = this.clone(Obj[i]);
  ...
  public static accMul(arg1: number, arg2: number): number {
    ...
    let m = 0,
      s1 = arg1.toFixed(8).replace(/0*$/g, ""),
      s2 = arg2.toFixed(8).replace(/0*$/g, "");
    try {
      m += s1.split(".")[1].length;
    } catch (e) {}
    try {
  ...
  public static timestampToDateTime(
    timestamp = Date.now(),
  ...
  public static isSelfDid(did: string) {
    if (
      did == DIDSessionsStore.signedInDIDString ||
      "did:elastos:" + did == DIDSessionsStore.signedInDIDString
    ) {
      return true;
    }
    return false;
  ...
  public static async getSelfPublicKey(): Promise<string> {
    const DIDService = await import(
      "src/app/identity/services/did.service"
    ).then((m) => m.DIDService);
    let base58Key = await DIDService.instance
      .getActiveDid()
      .getLocalDIDDocument()
      .getDefaultPublicKey();
