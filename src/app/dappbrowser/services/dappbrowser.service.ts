...
} from "src/app/model/ethereum/requestparams";
...
export class DappBrowserService implements GlobalService {
  ...
  public async getBrowseMode(): Promise<DAppsBrowseMode> {
    if (
      await this.prefs.getUseBuiltInBrowser(
   ...
   * on ios.
   */
  public async openForBrowseMode(
    url: string,
    title?: string,
    target?: string
  ): Promise<void> {
  ...
  public async open(
    ...
    if (activeNetwork instanceof EVMNetwork) {
      ...
      if (WalletService.instance.activeNetworkWallet.value) {
        // EVM configuration
        let evmSubwallet =
          WalletService.instance.activeNetworkWallet.value.getMainEvmSubWallet();
        if (evmSubwallet)
        ...
        this.userELAMainChainAddress = (
          await this.getWalletELAMainChainAddressesByType(masterWallet, 1)
        )?.[0];
      }
    } else {
      this.userEVMAddress = null;
  ...
  private async sendActiveWalletToDApp(networkWallet: AnyNetworkWallet) {
    ...
    if (networkWallet) {
      ...
      this.userBTCAddress = await this.getWalletBitcoinAddress(
        networkWallet.masterWallet
      );

      this.userELAMainChainAddress = (
        await this.getWalletELAMainChainAddressesByType(
          networkWallet.masterWallet,
          1
        )
      )?.[0];
  ...
  private async handleLoadStartEvent(
    event: DappBrowserPlugin.DappBrowserEvent
  ) {
  ...
  private handleLoadStopEvent(event: DABLoadStop): Promise<void> {
    if (!this.networkSubscription) {
      ...
      this.networkSubscription =
        WalletNetworkService.instance.activeNetwork.subscribe(
          (activeNetwork) => {
    ...
    if (!this.walletSubscription) {
      this.walletSubscription =
        WalletService.instance.activeNetworkWallet.subscribe((netWallet) => {
  ...
  private async extractHtmlInfoAndUpdatedBrowsedDApp(
    html: string,
    forUrl: string
  ): Promise<Document> {
    ...
    if (!title) {
      ...
      if (metas && metas.length > 0) {
        let appNameMeta = Array.from(metas).find(
          (m) => m.name && m.name.toLowerCase() === "application-name"
        );
    ...
    if (metas && metas.length > 0) {
      let descriptionMeta = Array.from(metas).find(
    ...
    if (links && links.length > 0) {
      let iconLink = Array.from(links).find(
        (l) => l.rel && l.rel.toLowerCase().indexOf("icon") >= 0
      );
      if (iconLink) {
    ...
    if (metas && metas.length > 0) {
      let themeColorMeta = Array.from(metas).find(
        (m) => m.name && m.name.toLowerCase() === "theme-color"
      );
  ...
  private async handleSendTransaction(message: DABMessage): Promise<void> {
    ...
    } = await GlobalIntentService.instance.sendIntent(
      "https://wallet.web3essentials.io/esctransaction",
      {
        payload: {
  ...
  private async handleSignTypedData(message: DABMessage): Promise<void> {
    ...
    let response: { result: SignTypedDataIntentResult } =
      await GlobalIntentService.instance.sendIntent(
  ...
  private async handlePersonalSign(message: DABMessage): Promise<void> {
    ...
    let response: { result: PersonalSignIntentResult } =
      await GlobalIntentService.instance.sendIntent(
  ...
  private async handleInsecureEthSign(message: DABMessage): Promise<void> {
    let rawData: { data: unknown } = message.data.object;
    let response: { result: EthSignIntentResult } =
      await GlobalIntentService.instance.sendIntent(
  ...
  private async handleSwitchEthereumChain(message: DABMessage): Promise<void> {
    ...
    if (!targetNetwork) {
      ...
      if (
        (
          WalletNetworkService.instance.activeNetwork.value as EVMNetwork
      ...
      let networkSwitched =
        await GlobalSwitchNetworkService.instance.promptSwitchToNetwork(
          targetNetwork
        );
      if (networkSwitched) {
  ...
  private async handleAddEthereumChain(message: DABMessage): Promise<void> {
    ...
    if (!existingNetwork) {
      ...
      let response: EditCustomNetworkIntentResult =
        await GlobalIntentService.instance.sendIntent(
    ...
    if (
      ...
      if (!targetNetwork)
        targetNetwork =
          WalletNetworkService.instance.getNetworkByKey(addedNetworkKey);
  ...
  private async handleElastosGetCredentials(
    message: DABMessage
  ): Promise<void> {
    try {
      ...
      res = await GlobalIntentService.instance.sendIntent(
        "https://did.web3essentials.io/credaccess",
        query
      );

      if (!res || !res.result || !res.result.presentation) {
        console.warn(
          "Missing presentation. The operation was maybe cancelled."
        );
  ...
  private async handleUnisatMessage(message: DABMessage) {
    ...
    switch (message.data.name) {
      ...
      default:
        Logger.warn(
          "dappbrowser",
          "Unhandled unisat message command",
          message.data.name
        );
    }
  }
  ...
  private async handleBitcoinPushTx(message: DABMessage): Promise<void> {
    try {
      let response: {
        ...
        "https://wallet.web3essentials.io/pushbitcointx",
        {
          payload: {
            params: [message.data.object],
          },
  ...
  private async handleBitcoinSend(message: DABMessage): Promise<void> {
    try {
      let response: {
        ...
        "https://wallet.web3essentials.io/sendbitcoin",
        {
          payload: {
            params: [message.data.object],
          },
  ...
  private async handleBitcoinSignMessage(message: DABMessage): Promise<void> {
    try {
      let responseSignMessage: {
        ...
        "https://wallet.web3essentials.io/signbitcoinmessage",
        {
          payload: {
            params: [message.data.object],
          },
      ...
      if (responseSignMessage.result.signature) {
        this.sendInjectedResponse(
          "unisat",
          message.data.id,
          responseSignMessage.result.signature
        );
      } else {
        this.sendInjectedError("unisat", message.data.id, {
          code: 4001,
  ...
  private async handleBitcoinSignData(message: DABMessage): Promise<void> {
    try {
      let responseSigndata: {
        ...
        "https://wallet.web3essentials.io/signbitcoindata",
        {
          payload: {
            params: [message.data.object],
          },
      ...
      if (responseSigndata.result.signature) {
        this.sendInjectedResponse(
          "unisat",
          message.data.id,
          responseSigndata.result.signature
        );
      } else {
        this.sendInjectedError("unisat", message.data.id, {
          code: 4001,
    ...
    switch (message.data.name) {
      ...
      case "elamain_signMessage":
        let response: {
          ...
          "https://wallet.web3essentials.io/elamainsignmessage",
          {
      ...
      default:
        Logger.warn(
          "dappbrowser",
          "Unhandled elamain message command",
          message.data.name
        );
    }
  }
  ...
  private async handleEssentialsUrlIntent(message: DABMessage): Promise<void> {
    try {
      ...
      if (!res || !res.result) {
        console.warn(
          "Missing response data. The operation was maybe cancelled."
        );
        this.sendInjectedError(
          "elastos",
          message.data.id,
  ...
  private sendInjectedError(
    ...
    if (provider === "elastos")
      stringifiedError =
        typeof error == "string" ? error : new String(error).toString();
   ...
   */
  public async saveBrowsedAppInfo(
    appInfo: BrowsedAppInfo
  ): Promise<BrowsedAppInfo> {
  ...
  private async addAppToRecent(url: string) {
    ...
    let existingIndex = this.recentApps.value.findIndex(
      (appUrl) => urlDomain(appUrl) === rootDomain
    );
  ...
  private async backgroundFetchUrlInfo(url: string): Promise<void> {
    ...
    try {
      ...
      Logger.warn(
        "dappbrowser",
        `Failed to fetch background url info for url ${url}`,
        e
      );
    }
  }
  ...
  private getBitcoinNetwork(): BTCMainNetNetwork {
    return this.walletNetworkService.getNetworkByKey(
      "btc"
    ) as BTCMainNetNetwork;
  }

  private async getWalletBitcoinAddress(
    masterWallet: MasterWallet
  ): Promise<string> {
    const bitcoinNetwork = this.getBitcoinNetwork();
    const bitcoinNetworkWallet = await bitcoinNetwork.createNetworkWallet(
      masterWallet,
      false
    );
    const addresses = bitcoinNetworkWallet?.safe.getAddresses(
      0,
      1,
      false,
      null
  ...
  private async getWalletBitcoinPublicKey(
    masterWallet: MasterWallet
  ): Promise<string> {
    const bitcoinNetwork = this.getBitcoinNetwork();
    const bitcoinNetworkWallet = await bitcoinNetwork.createNetworkWallet(
  ...
  private async getWalletELAMainChainAddressesByType(
    ...
    const elaMainChainNetworkWallet =
      await elaMainChainNetwork.createNetworkWallet(masterWallet, false);
    if (!elaMainChainNetworkWallet) return [];

    let elaSubwallet = elaMainChainNetworkWallet.getSubWallet(
    ...
    switch (type) {
      ...
      case AddressType.Normal_external:
        addressArray = elaMainChainNetworkWallet.safe.getAddresses(
          index,
          count,
          internal,
          null
      ...
      case AddressType.All:
        ...
        let addressesExternal = elaMainChainNetworkWallet.safe.getAddresses(
          index,
          addressAccount,
          true,
          null
        ...
        let addressesInternal = elaMainChainNetworkWallet.safe.getAddresses(
          index,
          addressAccount,
          false,
          null
