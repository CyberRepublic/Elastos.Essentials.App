(function () {
  ...
  */
   ...
   */
  class DappBrowserUnisatProvider extends events {
      constructor(rpcUrl, address) {
          super();
          this.rpcUrl = null;
          this.address = null; // Bitcoin address
          this.requests = new Map(); // stores on going requests
          this.isEssentials = true; // Let dapps know that this provider is the Essentials injected one, in case they want to adjust their UI.
      ...
      async sendBitcoin(payAddress, satAmount, options) {
       ...
       */
      async signData(rawData, type = "ecdsa") {
       ...
       */
      async executeRequest(name, data) {
          ...
          const result = new Promise((resolver, rejecter) => {
              // Rember the request
      ...
      postMessage(message) {
          ...
          window.webkit.messageHandlers.essentialsExtractor.postMessage(JSON.stringify(message));
      }
      /**
       * Internal native result -> js
       */
  ...
  (function (AddressType) {
      AddressType["Normal_external"] = "normal-external";
      AddressType["Normal_internal"] = "normal-internal";
      AddressType["Owner"] = "owner";
      AddressType["CROwnerDeposit"] = "cr-owner-deposit";
      AddressType["OwnerDeposit"] = "owner-deposit";
      AddressType["OwnerStake"] = "owner-stake";
      AddressType["All"] = "all";
   ...
   */
  class DappBrowserElaMainProvider extends events {
      constructor(rpcUrl, address) {
          super();
          this.rpcUrl = null;
          this.address = null; // Ela main chain address
          this.requests = new Map(); // stores on going requests
       ...
       * @returns
       */
      async getMultiAddresses(count, type = AddressType.All, index = 0) {
